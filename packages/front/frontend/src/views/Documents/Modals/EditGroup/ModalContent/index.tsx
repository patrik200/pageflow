...
function EditGroupModalContent({ group, close, onSuccess }: EditGroupModalContentInterface) {
  ...
  const { createGroup, updateGroup, filter } = useViewContext().containerInstance.get(DocumentStorage);

  const entity = React.useMemo(
    () => (group ? filter.getEditDocumentGroupEntity(group) : filter.getCreateDocumentGroupEntity()),
    [filter, group],
  );
