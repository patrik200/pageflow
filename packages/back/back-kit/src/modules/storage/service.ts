...
export class StorageControllerService {
  ...
  private async sendImageFileWithResize(res: Response, { bucket, id }: { bucket: string; id: string }, width: number) {
    const stream = await this.imageVariantsService.getVariantStreamOrCreate(bucket, id, [
      { key: "width", value: width },
    ]);
    if (!stream) return await this.sendDefaultFileStream(res, { bucket, id });
    StorageControllerService.setContentType(res, id);
    stream.pipe(res);
  }

  private async sendDefaultFileStream(res: Response, { id, bucket }: { bucket: string; id: string }) {
    const stream = await this.storageGetService.getFileStream({ bucket, id });
  ...
  async file(
    res: Response,
    options: { bucket: string; id: string },
    { width: imageVariantWidth }: { width?: string } = {},
  ) {
