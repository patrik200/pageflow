...
export class PaymentsCancellerService implements OnApplicationBootstrap, OnApplicationShutdown {
  ...
  private loggerContext = "Payments background canceller";
  ...
  private async checkPayments() {
    const payments = await this.getPaymentService.dangerGetPaymentsList([PaymentStatus.WAITING_FOR_PAYMENT]);

    await Promise.all(payments.map((payment) => this.checkPayment(payment)));
  }
